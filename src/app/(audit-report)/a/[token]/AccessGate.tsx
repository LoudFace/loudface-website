...
import { unlockAudit, type UnlockState } from './actions';

/**
 * The locked screen. It knows the token (already in the URL) and nothing else —
 * no company name, no findings. Everything a stranger could learn here, they
 * already had.
 */
export function AccessGate({ token }: { token: string }) {
  const [state, formAction, pending] = useActionState<UnlockState, FormData>(unlockAudit, {});
    ...
    >
      <div className="w-full max-w-sm">
        ...
        <form action={formAction} className="mt-8">
          ...
          <label htmlFor="audit-code" className="block text-xs uppercase tracking-[0.08em] text-white/50">
            Access code
          </label>
          <input
            id="audit-code"
            ...
            spellCheck={false}
            required
            aria-describedby={state.error ? 'audit-code-error' : undefined}
          ...
          {state.error && (
            <p id="audit-code-error" role="alert" className="mt-3 text-sm text-white/85">
              {state.error}
            </p>
          )}
          ...
          >
            {pending ? 'Checking…' : 'Open the audit'}
          </button>
        </form>
