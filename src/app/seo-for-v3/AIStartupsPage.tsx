...
const buyerQuestions = [
  ...
  ['What does your SOC 2 report actually cover?', 'The report itself, its scope, its period and its auditor, rather than a badge on a footer', 'A trust page that offers the report and states its scope'],
...
export const AI_STARTUPS_FAQ_ITEMS = [
  ...
  ['Does LoudFace have AI-native clients?', 'Yes. Bluefyn, an AI verification product, raised AI visibility more than 5x on a fixed set of tracked buyer prompts in under three months, with ChatGPT visibility up 13x and average position when named improving to under 2. Pond, an AI workforce marketplace, tripled AI visibility from a near-zero base in its first four weeks and went from absent to named on Google AI Mode. Those are visibility figures. Neither one is a revenue figure.'],
  ['Do comparison and review pages still matter for an AI startup?', 'Yes, and the review sites are algorithms rather than analyst opinions. G2 states that its score is computed from Satisfaction and Market Presence, that older reviews are weighted less, and that its reviews do not constitute expert opinions based on objective criteria. You control your own comparison and alternatives pages completely, so build those first and treat the review programme as continuous rather than a launch push.'],
  ...
  return (
    <>
      ...
      <section className="hr-prose" aria-label="SEO for AI startups">
        <div className="container hr-narrow">
          ...
          <Section title="Publish the accuracy evidence a developer will check">
            ...
            <p>The same rule governs your documentation surface. Hugging Face defines the model card as the README of a model repository and says what it should describe: the model, &quot;its intended uses &amp; potential limitations, including biases and ethical considerations&quot;, the training parameters, the datasets used to train the model, and &quot;the model&apos;s evaluation results&quot;. It also notes that the YAML metadata block is what makes the model findable at all. GitHub says the same thing about repositories: &quot;A README is often the first item a visitor will see when visiting your repository&quot;, and topics exist so &quot;other people find and contribute to your project&quot;. For an AI startup, the model card and the README are search assets rather than chores.</p>
          </Section>
          <Section title="Name the company so an engine can tell it apart">
            <p>An AI startup named after a common noun is competing with the dictionary. Google documents the fix. Organization structured data on the home page helps Google understand the organization, and the disambiguation property is sameAs, which Google defines as &quot;The URL of a page on another website with additional information about your organization&quot;. Pair that with one category sentence repeated on every surface you control. An engine can only repeat language it can find. Our guide to <Link href="/blog/entity-disambiguation-b2b-saas">entity disambiguation</Link> covers the work in full.</p>
            <p>Comparison pages are the other surface you own outright. The review sites are algorithms rather than analysts: G2 states that &quot;A software&apos;s G2 Score is calculated using two scoring components: Satisfaction and Market Presence&quot;, that &quot;Because recent reviews are more relevant to buyers, older reviews are weighted less&quot;, and that its reviews &quot;do not constitute expert opinions based on objective criteria&quot;. You cannot edit that. You can publish your own comparison and alternatives pages, and they are the format AI engines quote most often.</p>
          </Section>
        </div>
      </section>
      ...
      <section className="hr-feature hr-migration">
        <div>
          <h2>Proof from AI-native clients we run</h2>
          <p>Bluefyn (bluefyn.ai) builds &quot;The Proof Layer&quot;, an AI verification product. On a fixed set of tracked buyer prompts, comparing the first 30 days of the programme against the latest 30 days, AI visibility rose more than 5x and share of voice rose more than 5x against an unfiltered competitor set. ChatGPT visibility rose 13x. Average position when named improved to under 2. Under three months separates the two windows.</p>
          <p>Pond (joinpond.ai) runs an AI workforce marketplace. AI visibility tripled from a near-zero base in the first four weeks, and Pond went from absent to named on Google AI Mode. The base is small and the programme is four weeks old, so read it as a standing start rather than a large result.</p>
          <p>Both companies sell an AI product rather than software that uses AI internally, so both sit inside the definition this page uses. Outside the AI-native vertical, the <Link href="/case-studies/stealth-fintech-ai-visibility">Stealth Fintech</Link> and <Link href="/case-studies/genie-teacher-organic-growth">Genie Teacher</Link> case studies document the same method in other categories. Every figure above is a visibility figure. None of them is a revenue figure.</p>
        </div>
        <div className="hr-feature-image ais-panel">
          <h3>Pond, first four weeks</h3>
          <ul className="ais-stats">
            ...
            <li><b>Absent to named</b><span>Google AI Mode, in the same four weeks</span></li>
          </ul>
        </div>
      </section>
