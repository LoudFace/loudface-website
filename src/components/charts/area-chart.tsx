...
function ChartInner({
  ...
  /*
   * LOCAL FIX — not upstream. `clipPathId` was the hardcoded string
   * "chart-area-grow-clip", so two AreaCharts on one page emitted the same SVG
   * id. SVG resolves `url(#id)` to the FIRST match in the document, so the
   * second chart was clipped by the first chart's reveal rect — its line stopped
   * partway across and never finished drawing. Measured on the TradeMomentum
   * case study, 2026-08-31: the Google chart was clipped to 335px inside a 766px
   * plot.
   *
   * `useId()` gives each instance its own id. The library already does exactly
   * this for its gradient defs, so the hardcoded clip is an upstream oversight
   * rather than a design choice — worth reporting, and worth re-applying if this
   * component is ever re-fetched from the @bklit registry.
   */
