# Beautiful UI (beautifui.dev) — Full Harvest

Source: https://www.beautifului.dev/ — fetched via `curl` (raw CSS + shadcn-style registry JSON), no summarization/WebFetch used for code or CSS.

**Correction of an earlier shallow pass:** that pass wrongly concluded this site had "nothing." It does — the site ships a real shadcn-compatible registry at `/r/registry.json`, and a hand-authored, uncompiled `foundation.css` with full light+dark design tokens. Both are captured verbatim below.

## Table of Contents

- [Part A — Design Token System](#part-a--design-token-system)
  - [A1. Source files](#a1-source-files)
  - [A2. Colour tokens (light + dark)](#a2-colour-tokens-light--dark)
  - [A3. Type system](#a3-type-system)
  - [A4. Radii](#a4-radii)
  - [A5. Shadows](#a5-shadows)
  - [A6. Easing / motion](#a6-easing--motion)
  - [A7. Keyframes](#a7-keyframes)
  - [A8. Base rules / interface hygiene](#a8-base-rules--interface-hygiene)
  - [A9. Full verbatim foundation.css](#a9-full-verbatim-foundationcss)
  - [A10. Compiled homepage CSS cross-check](#a10-compiled-homepage-css-cross-check)
  - [A11. Not part of the design system](#a11-not-part-of-the-design-system)
- [Part B — Component Registry](#part-b--component-registry)
  - [Beautiful UI foundation (`foundation`)](#foundation)
  - [Button (`button`)](#button)
  - [GlideMenu (`glide-menu`)](#glide-menu)
  - [EntityChip (`entity-chip`)](#entity-chip)
  - [ValuePill (`value-pill`)](#value-pill)
  - [Shimmer (`shimmer`)](#shimmer)
  - [StreamText (`stream-text`)](#stream-text)
  - [Loading State (`loading-state`)](#loading-state)
  - [Thinking (`thinking-state`)](#thinking-state)
  - [Streaming Text (`streaming-text`)](#streaming-text)
  - [Approval Card (`approval-card`)](#approval-card)
  - [Tool Chips (`tool-chips`)](#tool-chips)
  - [Task Rows (`task-rows`)](#task-rows)
  - [Chat (`chat-composer`)](#chat-composer)
  - [Prompt Bar (`prompt-bar`)](#prompt-bar)
  - [Recommendation Card (`recommendation-card`)](#recommendation-card)
  - [Context Cards (`context-cards`)](#context-cards)
  - [Diff Table (`diff-table`)](#diff-table)
  - [Records Table (`records-table`)](#records-table)
  - [Filter Table (`filter-table`)](#filter-table)
  - [Sidebar Nav (`sidebar-nav`)](#sidebar-nav)
  - [Search (`search`)](#search)
  - [Flowchart (`flowchart`)](#flowchart)
  - [Insight Cards (`insight-cards`)](#insight-cards)
  - [Code Block (`code-block`)](#code-block)
  - [Fine-tune Card (`fine-tune-card`)](#fine-tune-card)
  - [Selection Actions (`selection-actions`)](#selection-actions)

## Part A — Design Token System

### A1. Source files

| File | How it was fetched | Purpose |
|---|---|---|
| `/r/foundation.json` -> `app/beautifui/foundation.css` | `curl https://www.beautifului.dev/r/foundation.json`, JSON `files[0].content` | The hand-authored, uncompiled source of truth. Full `:root` + `.dark` token sets, `@theme inline` mapping, base rules, shared keyframes, and per-component CSS blocks (clearly commented `COMPONENT-SPECIFIC`). |
| `/_next/static/css/91256ae11d6830cd.css` | `curl` on the homepage's own `<link rel="stylesheet">` href | Tailwind v4 compiled output (88,853 bytes) — confirms the same token values ship to production and shows which utility classes the homepage actually renders. |
| `/_next/static/css/6c593f1e66a64262.css` | same method | A third-party dev-tools overlay widget's CSS ("dialkit") — NOT part of Beautiful UI's own design system. See A11. |
| `app/beautifui/records-table.css`, `app/beautifui/sidebar-nav.css` | `curl` on `/r/records-table.json` and `/r/sidebar-nav.json`, extra CSS file entries | Component-specific CSS shipped alongside two components (records grid ~16.5KB, sidebar ~2KB). Captured in full in Part B. |

Font files (`@font-face`, self-hosted via `next/font`): **Inter** (variable, 7 unicode-range subsets + local Arial fallback with size-adjust) and **JetBrains Mono** (variable, 7 subsets + fallback). Confirmed from the compiled CSS's `@font-face` rules — the raw `.woff2` binaries were not downloaded (not needed for token capture).

### A2. Colour tokens (light + dark)

Colour space: **oklch** throughout (`oklch(L C H)` and `oklch(L C H / alpha)`), plus a small number of `color-mix(in srgb, …)` derivations layered on top of oklch base tokens, and hex used only inside shadow-plugin's shadow-color slots (`#0000000a` etc., not tokens). This is read directly from `foundation.css` — not converted or approximated.

**Light (`:root`)**

| Token | Value | Role |
|---|---|---|
| `--page` | `oklch(0.985 0.001 286.376)` | page background |
| `--canvas` | `oklch(0.961 0.002 247.84)` | canvas/app background |
| `--surface` | `oklch(1 0 0)` | card/surface white |
| `--inset` | `oklch(0.979 0.002 247.839)` | inset wells |
| `--hover` | `oklch(0.97 0.002 247.839)` | hover fill |
| `--hover-2` | `oklch(0.933 0.003 247.86)` | stronger hover fill |
| `--ink` | `oklch(0.247 0.006 258.361)` | primary text |
| `--ink-2` | `oklch(0.506 0.01 264.477)` | secondary text |
| `--ink-3` | `oklch(0.695 0.009 264.505)` | tertiary/muted text |
| `--line` | `oklch(0.946 0.003 264.542)` | default hairline border |
| `--line-strong` | `oklch(0.912 0.005 258.326)` | stronger border |
| `--line-soft` | `oklch(0.966 0.002 264.542)` | soft border |
| `--grid-line` | `color-mix(in srgb, var(--line) 78%, transparent)` | grid rule derived from --line |
| `--field` | `oklch(0.961 0.001 286.375)` | input field background |
| `--stripe` | `oklch(0.405 0 0 / 0.075)` | diagonal body-background stripe |
| `--stripe-bg` | `oklch(0.97 0 0)` | stripe base background |
| `--accent` | `oklch(0.626 0.205 254.947)` | brand accent blue |
| `--accent-ink` | `oklch(0.556 0.187 255.617)` | accent text-on-accent |
| `--accent-tint` | `oklch(0.96 0.019 252.878)` | accent tint/selection bg |
| `--green` | `oklch(0.603 0.155 150.883)` | success |
| `--green-tint` | `oklch(0.958 0.017 159.118)` | success tint |
| `--orange` | `oklch(0.689 0.179 49.902)` | warning |
| `--orange-tint` | `oklch(0.964 0.021 67.581)` | warning tint |
| `--red` | `oklch(0.621 0.192 23.042)` | error/danger |
| `--red-tint` | `oklch(0.956 0.017 17.462)` | error tint |
| `--tooltip-bg` | `oklch(0.272 0.008 264.435)` | chart tooltip background |
| `--tooltip-fg` | `oklch(0.976 0.002 247.839)` | chart tooltip text |
| `--tooltip-muted` | `oklch(0.731 0.008 260.731)` | chart tooltip muted text |
| `--tooltip-border` | `oklch(0.356 0.007 264.474)` | chart tooltip border |

**Dark (`.dark`)** — full override set, class-based (no `prefers-color-scheme` media query in the source; theme is toggled by a `.dark` class + `localStorage`, per the inline script seen in the raw HTML):
...
Note: `insight-cards`/`filter-table`/`records-table` add a few extra hue tokens local to those components (e.g. `--tag-base` per status pill) — those live in their own CSS blocks, documented in Part B, not in the shared foundation.

### A3. Type system

**Font families** (from `@theme inline` in foundation.css + compiled `@font-face`):

| Token | Stack |
|---|---|
| `--font-sans` | `var(--font-inter), ui-sans-serif, system-ui, sans-serif` (Inter, self-hosted variable font via next/font) |
| `--font-mono` | `var(--font-mono-face), ui-monospace, "SF Mono", monospace` — `--font-mono-face` resolves to `"JetBrains Mono", "JetBrains Mono Fallback"` |

**Body defaults** (from foundation.css `body{}`): `font-size: 14px`, `letter-spacing: -0.01em`, `line-height: 1.5`, `font-feature-settings: "cv11", "ss01"` (Inter's single-story a / straight-leg r stylistic sets).

**Explicit type scale is NOT declared as a named scale in `foundation.css`** — Beautiful UI relies on Tailwind v4's default `--text-*` scale plus ad hoc pixel values inside individual component files (e.g. the chart tooltip CSS uses `font-size: 11.5px` directly). What the compiled homepage CSS actually generated as utility classes (i.e. what's used on the homepage itself — a partial view, since Tailwind v4 only emits classes that are used):

| Utility | font-size | line-height |
|---|---|---|
| `.text-xs` | `.75rem` (12px) | `calc(1 / .75)` |
| `.text-sm` | `.875rem` (14px) | `calc(1.25 / .875)` |
| `.text-2xl` | `1.5rem` (24px) | `calc(2 / 1.5)` |
Stated explicitly: this is what I could confirm from their CSS. `.text-base`, `.text-lg`, `.text-xl`, `.text-3xl+` were not found emitted in the two stylesheets fetched (`/` and its linked CSS) — either the homepage doesn't use those sizes, or another page emits them and I did not find another stylesheet link to check.

**Font weights used**: `class-variance-authority` variants inside `Button.tsx` (Part B) use `font-medium` (500). foundation.css does not declare its own weight scale — it inherits Tailwind's default `--font-weight-*` tokens (`normal:400 / medium:500 / semibold:600 / bold:700`), confirmed present in the compiled CSS's base `:root`.

### A4. Radii

From `@theme inline` in foundation.css — a **named, purpose-based radius scale** (not a generic sm/md/lg scale):

| Token | Value | Use |
|---|---|---|
| `--radius-chip` | `6px` | chips |
| `--radius-control` | `8px` | controls (buttons, inputs, icon buttons) |
| `--radius-card` | `10px` | cards |
| `--radius-window` | `14px` | window/panel-level containers |
| (pill) | `rounded-full` / `3.40282e+38px` | Tailwind's synthetic "infinite" radius for pills, confirmed in compiled CSS |

### A5. Shadows

Two shadow systems compose together:

1. **`shadow-plugin/unprefixed`** (a third-party Tailwind plugin, imported via `@import "shadow-plugin/unprefixed";`) supplies smooth, multi-layer `--shadow-xs` … `--shadow-lg` values. Captured from the compiled CSS's base `:root` (this plugin's own output, not authored in foundation.css):

```css
--shadow-xs: 0 0 4px 0 #0000000a;
--shadow-sm: 0 18px 47px 0 #00000008, 0 7.5px 19px 0 #00000005, 0 4px 10.5px 0 #00000005, 0 2.3px 5.8px 0 #00000003, 0 1.2px 3.1px 0 #00000003, 0 .5px 1.3px 0 #00000003;
--shadow-md: 0 17.54px 23.39px 0 #0000000a, 0 9.4px 12.5px 0 #00000008, 0 5.25px 7px 0 #00000005, 0 2.79px 3.72px -2px #00000003, 0 1.16px 1.5px 0 #00000003;
...
```
2. **Beautiful UI's own semantic shadow tokens** (foundation.css) build a solid hairline ring on top of those layered blurs — light and dark diverge (dark switches the ring from a dark line to a translucent *light* hairline so it reads against a dark surface):
...
```
No standalone blur token scale was found — blur values appear inline per-use (`filter: blur(1.6px)` in `.stream-tail`; the unrelated dialkit widget has its own `--dial-backdrop-blur:20px`, not part of this system).

### A6. Easing / motion

| Token | Value | Note |
|---|---|---|
| `--ease-out-strong` | `cubic-bezier(0.23, 1, 0.32, 1)` | "built-ins are too weak" — author's own code comment |
| `--ease-in-out-strong` | `cubic-bezier(0.77, 0, 0.175, 1)` | |
| `--ease-link` | `cubic-bezier(0.16, 1, 0.3, 1)` | used for the `.animated-underline` link hover |
| `--ease-out` / `--ease-in-out` | `cubic-bezier(0,0,.2,1)` / `cubic-bezier(.4,0,.2,1)` | Tailwind v4 defaults, present in compiled CSS, not redeclared in foundation.css |
Durations found inline (no shared duration tokens): `280ms` (underline sweep), `1s` (caret blink). Theme-switch freeze: `.theme-switching * { transition: none !important; }` so a light/dark flip repaints as one clean frame instead of hundreds of mismatched fades.

### A7. Keyframes

All captured verbatim from foundation.css:

```css
@keyframes shimmer-text { from { background-position: 150% center; } to { background-position: -50% center; } }
...
```
Plus `records-pulse` (compiled CSS only, belongs to `records-table.css`, not general foundation):
```css
@keyframes records-pulse { 0%, to { opacity: .35; transform: scale(.8); } 50% { opacity: 1; transform: scale(1); } }
```

### A8. Base rules / interface hygiene

Highlights worth reusing, verbatim from foundation.css:

- `html { -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; text-rendering: optimizeLegibility; }`
- Body has a fixed 45° diagonal stripe texture (`background-attachment: fixed`, 1px stripe every 8px) using `--stripe` / `--stripe-bg` tokens — a deliberate paper-like canvas texture, not a plain flat fill.
- `::selection { background: var(--accent-tint); color: var(--ink); }`
- `h1,h2,h3,h4 { text-wrap: balance; }`, `p { text-wrap: pretty; }` — same rule this project's own `styling.md` already documents.
- `a { text-underline-position: from-font; text-decoration-skip-ink: auto; }`
- `:focus-visible { outline: 2px solid var(--accent); outline-offset: 2px; border-radius: 4px; }`; inputs/textareas/selects suppress the default outline (styled another way instead).
- `@media (prefers-reduced-motion: reduce)` zeroes all animation/transition durations globally as a final safety net, on top of per-component reduced-motion overrides (e.g. the streaming caret).
- `@custom-variant dark (&:where(.dark, .dark *));` — Tailwind v4 custom variant wiring `dark:` utilities to a `.dark` class rather than `prefers-color-scheme`.

### A9. Full verbatim foundation.css

Fetched with `curl https://www.beautifului.dev/r/foundation.json`, this is the `files[0].content` field, reproduced in full (636 lines, 16,736 characters) — the actual shipped source, not a paraphrase:

```css
@import "tailwindcss";
/* shadow-plugin (flornkm) — smooth, layered shadow stacks. The unprefixed
 * entry maps them onto Tailwind's --shadow-xs…2xl scale; our semantic tokens
 * below build on those values so the whole app picks up the smooth shadows. */
@import "shadow-plugin/unprefixed";

@custom-variant dark (&:where(.dark, .dark *));

/* GENERIC FOUNDATION — tokens, @theme, base rules, the primitive-*
...
body {
  ...
  /* Pin the stripe pattern to the viewport so it stays put on scroll. */
...
/* ── interface hygiene (interfaces.dev cheat sheet) ────── */

/* headings balance; descriptions avoid orphans (skip long-form — we have none) */
h1,
h2,
h3,
h4 {
...
/* theme switches swap every token at once — freeze transitions so the flip
 * is one clean repaint instead of hundreds of mismatched color fades */
.theme-switching *,
...
/* touch: :hover sticks after a tap, so neutralize row/header hovers there */
@media (hover: none) {
...
/* Keep the component nav scrollable but hide its scrollbar. */
.component-nav-scroll {
  scrollbar-width: none; /* Firefox */
  -ms-overflow-style: none; /* legacy Edge */
}
.component-nav-scroll::-webkit-scrollbar {
  display: none; /* WebKit */
}
...
/* Mobile demo gestures should operate controls and visualizations rather than
 * selecting surrounding labels. Text entry and code remain intentionally
 * selectable. */
@media (hover: none) and (pointer: coarse) {
  ...
  .primitive-showcase :is(input, textarea, [contenteditable="true"], pre, code) {
    -webkit-user-select: text;
    user-select: text;
    -webkit-touch-callout: default;
  }

  .primitive-showcase :is(a, button, [role="button"]) {
    -webkit-tap-highlight-color: transparent;
  }

  .primitive-showcase :is(a, button, [role="button"]):focus-visible {
    outline: none;
  }

  .primitive-showcase :is(img, svg, canvas) {
...
/* Tailwind v4 resets buttons to cursor:default — anything
 * clickable must invite the click */
button:not(:disabled),
...
.animated-underline::after {
  ...
  transition: transform 280ms var(--ease-link);
}

a:hover .animated-underline::after,
a:focus-visible .animated-underline::after {
  transform: scaleX(1);
}
...
.dark .records-tag,
.dark .filter-status-todo,
.dark .filter-status-progress,
.dark .filter-status-done {
...
### A10. Compiled homepage CSS cross-check

The production homepage stylesheet (`/_next/static/css/91256ae11d6830cd.css`, Tailwind v4.3.3, 88,853 bytes) was diffed by hand against foundation.css: every `--page/--canvas/--surface/--ink*/--line*/--accent*/--green*/--orange*/--red*` value in the compiled `:root` matches foundation.css's light tokens exactly, down to the oklch decimals. This confirms foundation.css is the real, current source — not a stale or aspirational doc.

One extra value found only in the compiled CSS (not part of the semantic tokens): `--smooth-ring-color` fed by `shadow-plugin`, resolved via `light-dark(#0000000d,#ffffff2e)` — a raw CSS `light-dark()` function call, separate from the `.dark`-class system used everywhere else on the site (likely internal to the shadow-plugin dependency itself, not house convention).

### A11. Not part of the design system

`/_next/static/css/6c593f1e66a64262.css` (38,053 bytes) is a **separate, unrelated stylesheet** for a floating dev-tools panel (class prefix `.dialkit-*`, e.g. `.dialkit-root`, `.dialkit-panel`, `.dialkit-folder`). It declares its own `--dial-*` custom properties (surface/text/border/shadow tokens) and imports Geist Mono from Google Fonts — a completely separate palette and font from Beautiful UI's own Inter/JetBrains Mono system. This is almost certainly a debugging/tweak-panel library bundled into the page, not house design tokens. Flagging this explicitly so it isn't mistaken for house tokens.

## Part B — Component Registry

Registry root: `https://www.beautifului.dev/r/registry.json` — a `$schema: https://ui.shadcn.com/schema/registry.json` shadcn-compatible registry, name `beautifui`. It was NOT linked from any page's HTML (no `npx shadcn add` snippet was found anywhere in the homepage, `/harness`, or `/license` markup or RSC payload) — it was found by guessing the standard shadcn registry-index path `/r/registry.json`, which returned `200`. Every individual item then resolved at `https://www.beautifului.dev/r/<name>.json` — the exact pattern shadcn's own CLI expects.

**27 items**, all fetched successfully (HTTP 200), all with complete, unmodified, unabridged source below. `foundation` (type `registry:style`) is documented in Part A; the other 26 are documented here.

### Component index

| Name | Title | Type | Registry deps | npm deps |
|---|---|---|---|---|
| `foundation` | Beautiful UI foundation | registry:style | — | — |
| `button` | Button | registry:ui | foundation.json | class-variance-authority |
| `glide-menu` | GlideMenu | registry:ui | foundation.json | — |
| `entity-chip` | EntityChip | registry:ui | foundation.json | — |
| `value-pill` | ValuePill | registry:ui | foundation.json | — |
| `shimmer` | Shimmer | registry:ui | foundation.json | — |
| `stream-text` | StreamText | registry:ui | foundation.json | — |
| `loading-state` | Loading State | registry:component | foundation.json | — |
| `thinking-state` | Thinking | registry:component | foundation.json | — |
| `streaming-text` | Streaming Text | registry:component | foundation.json | — |
| `approval-card` | Approval Card | registry:component | foundation.json, button.json, glide-menu.json | — |
| `tool-chips` | Tool Chips | registry:component | foundation.json | — |
| `task-rows` | Task Rows | registry:component | foundation.json | — |
| `chat-composer` | Chat | registry:component | foundation.json | — |
| `prompt-bar` | Prompt Bar | registry:component | foundation.json | glimm |
| `recommendation-card` | Recommendation Card | registry:component | foundation.json, button.json, entity-chip.json, value-pill.json | — |
| `context-cards` | Context Cards | registry:component | foundation.json | — |
| `diff-table` | Diff Table | registry:component | foundation.json, button.json | — |
| `records-table` | Records Table | registry:component | foundation.json, glide-menu.json | — |
| `filter-table` | Filter Table | registry:component | foundation.json | — |
| `sidebar-nav` | Sidebar Nav | registry:component | foundation.json, glide-menu.json | @central-icons-react/round-outlined-radius-2-stroke-2 |
| `search` | Search | registry:component | foundation.json, glide-menu.json | — |
| `flowchart` | Flowchart | registry:component | foundation.json | — |
| `insight-cards` | Insight Cards | registry:component | foundation.json | liveline |
| `code-block` | Code Block | registry:component | foundation.json | — |
| `fine-tune-card` | Fine-tune Card | registry:component | foundation.json, glide-menu.json | — |
| `selection-actions` | Selection Actions | registry:component | foundation.json, button.json, shimmer.json, stream-text.json | iconoir-react |

Install command pattern (standard shadcn CLI, confirmed working against this registry):
...
- Type: `registry:ui`
- Registry deps: https://www.beautifului.dev/r/foundation.json
- npm deps: class-variance-authority

**`components/atoms/Button.tsx`** (registry:ui, 1928 chars)
...
**`components/primitives/GlideMenu.tsx`** (registry:ui, 1920 chars)
...
**`components/atoms/EntityChip.tsx`** (registry:ui, 1275 chars)
...
**`components/atoms/ValuePill.tsx`** (registry:ui, 1251 chars)
...
**`components/atoms/Shimmer.tsx`** (registry:ui, 545 chars)
...
**`components/atoms/StreamText.tsx`** (registry:ui, 2284 chars)
...
**`components/primitives/LoadingState.tsx`** (registry:component, 4820 chars)
...
**`components/primitives/ThinkingState.tsx`** (registry:component, 10740 chars)
...
**`components/primitives/StreamingText.tsx`** (registry:component, 10289 chars)
...
- Type: `registry:component`
- Registry deps: https://www.beautifului.dev/r/foundation.json, https://www.beautifului.dev/r/button.json, https://www.beautifului.dev/r/glide-menu.json

**`components/primitives/ApprovalCard.tsx`** (registry:component, 16504 chars)
...
**`components/primitives/ToolChips.tsx`** (registry:component, 13967 chars)
...
**`components/primitives/TaskRows.tsx`** (registry:component, 10040 chars)
...
**`components/primitives/ChatComposer.tsx`** (registry:component, 8463 chars)
...
**`components/primitives/PromptBar.tsx`** (registry:component, 30493 chars)
...
- Type: `registry:component`
- Registry deps: https://www.beautifului.dev/r/foundation.json, https://www.beautifului.dev/r/button.json, https://www.beautifului.dev/r/entity-chip.json, https://www.beautifului.dev/r/value-pill.json

**`components/primitives/RecommendationCard.tsx`** (registry:component, 5812 chars)
...
**`components/primitives/ContextCards.tsx`** (registry:component, 4234 chars)
...
- Type: `registry:component`
- Registry deps: https://www.beautifului.dev/r/foundation.json, https://www.beautifului.dev/r/button.json

**`components/primitives/DiffTable.tsx`** (registry:component, 11380 chars)
...
- Type: `registry:component`
- Registry deps: https://www.beautifului.dev/r/foundation.json, https://www.beautifului.dev/r/glide-menu.json

**`components/primitives/RecordsTable.tsx`** (registry:component, 59535 chars)
...
**`app/beautifui/records-table.css`** (registry:file, 16503 chars)
...
**`components/primitives/FilterTable.tsx`** (registry:component, 6006 chars)
...
- Type: `registry:component`
- Registry deps: https://www.beautifului.dev/r/foundation.json, https://www.beautifului.dev/r/glide-menu.json
- npm deps: @central-icons-react/round-outlined-radius-2-stroke-2

**`components/primitives/SidebarNav.tsx`** (registry:component, 18307 chars)
...
**`app/beautifui/sidebar-nav.css`** (registry:file, 2024 chars)
...
**`components/primitives/SearchList.tsx`** (registry:component, 4387 chars)
...
**`components/primitives/Flowchart.tsx`** (registry:component, 17354 chars)
...
**`components/primitives/InsightCards.tsx`** (registry:component, 19944 chars)
...
**`components/primitives/CodeBlock.tsx`** (registry:component, 9833 chars)
...
**`components/primitives/FineTuneCard.tsx`** (registry:component, 12135 chars)
...
- Type: `registry:component`
- Registry deps: https://www.beautifului.dev/r/foundation.json, https://www.beautifului.dev/r/button.json, https://www.beautifului.dev/r/shimmer.json, https://www.beautifului.dev/r/stream-text.json
- npm deps: iconoir-react

**`components/primitives/SelectionActions.tsx`** (registry:component, 19567 chars)
