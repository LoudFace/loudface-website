...
| `AskAiBlock` / `StandingBlock` / `ForecastBlock` / `TracksBlock` / `GateBlock` / `MonthsBlock` | `ProposalBlocks.tsx` | Server components — the client-first body (2026-09-05). Each block opens with the CLIENT's own numbers and only then says what we do; the house rule is that no more than a third of a proposal's sections may be about LoudFace. `StandingBlock` reuses `StatChip` for the numbers. `TracksBlock` and `MonthsBlock` keep their supporting facts to one line each: a long list here was rejected as cognitive load. |
| `ProposalAskAi` / `ProposalGapChart` / `ProposalForecast` | `ProposalInteractive.tsx` | Client components, Bklit charts on the `proposal-plot-sm` height. `ProposalAskAi` is a pill-per-buyer-question tab strip over one bar chart; `ProposalGapChart` is citations by page type with the client's coverage as inline chips. `ProposalForecast` is TWO sliders and ONE adapting bar chart of **leads per paid month** — never a term total and never cost per lead (Arnel, 2026-09-05). Conversion rate is a printed assumption, not a third control, and every assumption prints under the chart. |
